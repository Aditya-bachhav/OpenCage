...
from cage_env.runner import compare_policies, load_session_rows, run_session
...
def test_reproducible_runs_and_controller_comparison(tmp_path: Path):
    ...
    result_a = run_session(seed=21, steps=80, log_path=log_a, policy_name="emergent")
    result_b = run_session(seed=21, steps=80, log_path=log_b, policy_name="emergent")
    ...
    comparison = compare_policies(seeds=[21, 22], steps=40, log_dir=tmp_path / "comparison")
